...
export function KnowledgePage() {
  ...
  const words = useMemo(() => {
    ...
    return vocabulary.filter((item) => !normalized || `${item.word} ${item.meaningZh}`.toLowerCase().includes(normalized)).slice(0, 60);
  }, [query]);
  ...
  return <section className="knowledge-page">
    ...
    {tab === 'vocabulary' && <><label className="knowledge-search">搜索高频词<input type="search" aria-label="搜索高频词" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="输入英文或中文释义" /></label><div className="word-grid">{words.map((item) => <article key={item.id} className="word-card"><span>词频 {item.frequency}</span><h2>{item.word}</h2><p className="phonetic">{item.phonetic}</p><p>{item.meaningZh}</p><small>{item.example}</small></article>)}</div>{words.length === 0 && <p className="empty-result">没有匹配结果，试试更短的关键词。</p>}</>}

...
export function TodayPage({ today = studyDate(), examDate, repository = defaultRepository }: { today?: string; examDate?: string; repository?: LearningRepository }) {
  ...
  return <div className="today-page">
    ...
    <section className="dashboard-hero"><div className="focus-card"><span>今日重点</span><h2>{focusKind ? `优先加强${taskCopy[focusKind].title}` : '先建立学习记录，再定位薄弱项'}</h2><p>先完成单词热身，再翻译题目并作答；错译的高频词会自动加入待掌握清单。</p><a className="focus-action" href={`${import.meta.env.BASE_URL}practice/vocabulary`}>先学高频词 →</a><a className="knowledge-action" href={`${import.meta.env.BASE_URL}knowledge`}>查看高频知识</a></div><div className="countdown-card"><span>距离考试</span><strong>{daysUntil(today, targetDate)}</strong><b>天</b><h3>{plan.phase === 'foundation' ? '基础补强期' : plan.phase === 'breakthrough' ? '题型突破期' : '冲刺模拟期'}</h3></div></section>

...
export function TodayPage({ today = new Date().toISOString().slice(0, 10), examDate, repository = defaultRepository }: { today?: string; examDate?: string; repository?: LearningRepository }) {
  ...
  return <div className="today-page">
    ...
    <section className="dashboard-hero"><div className="focus-card"><span>今日重点</span><h2>{metrics.weakSkill ? `优先加强${taskCopy[weakKind ?? 'listening'].title}` : '先建立学习记录，再定位薄弱项'}</h2><p>完成练习后自动记录，并用 3 道针对性题目确认是否真正掌握。</p><a className="focus-action" href={`${import.meta.env.BASE_URL}listen`}>开始今日训练 →</a><a className="knowledge-action" href={`${import.meta.env.BASE_URL}knowledge`}>先看高频知识</a></div><div className="countdown-card"><span>距离考试</span><strong>{daysUntil(today, targetDate)}</strong><b>天</b><h3>{plan.phase === 'foundation' ? '基础补强期' : plan.phase === 'breakthrough' ? '题型突破期' : '冲刺模拟期'}</h3></div></section>

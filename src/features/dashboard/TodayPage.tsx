...
export function TodayPage({ today = new Date().toISOString().slice(0, 10), examDate = '2026-12-12' }: { today?: string; examDate?: string }) {
  ...
  return <div className="today-page"><header className="page-heading"><div><h1>下午好，继续向 425 分前进</h1><p>今天只需要专注 60 分钟。</p></div><span className="avatar">L</span></header><section className="dashboard-hero"><div className="focus-card"><span>今日重点</span><h2>先听懂转折，再做对细节题</h2><p>关注 however、but、actually 后的信息</p><button>开始今日训练 →</button></div><div className="countdown-card"><span>距离考试</span><strong>{daysUntil(today, examDate)}</strong><b>天</b><h3>{plan.phase === 'foundation' ? '基础补强期' : plan.phase === 'breakthrough' ? '题型突破期' : '冲刺模拟期'}</h3></div></section><section className="dashboard-grid"><div className="task-panel"><h2>今日 60 分钟计划</h2>{plan.tasks.map((task) => { const copy = taskCopy[task.kind]; return <article key={task.id} className="task-card"><span className="task-icon">{copy.icon}</span><div><h3>{copy.title}</h3><p>{copy.detail}</p></div><b>{task.minutes} 分钟</b></article>; })}</div><ProgressCards /></section></div>;
}

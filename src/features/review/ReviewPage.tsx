...
export function ReviewPage({ repository = defaultRepository, now = new Date().toISOString(), examDate = '2026-12-12' }: { repository?: LearningRepository; now?: string; examDate?: string }) {
  ...
  const submit = async () => {
    ...
    const schedule = scheduleReviewStage(activeCard.stage, graded.correct, now.slice(0, 10), examDate);
    ...
    await completeDailyTask(repository, 'review', now.slice(0, 10));

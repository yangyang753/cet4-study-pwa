...
import { recordMeaningResult, recordSpellingResult } from '../vocabulary/wordMastery';
...
const filterKind = (card: ReviewCard) => {
  if (card.format === 'word-cloze' || card.wordId) return '词汇';
...
export function ReviewPage({ repository = defaultRepository, now = new Date().toISOString(), examDate }: { repository?: LearningRepository; now?: string; examDate?: string }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (activeCard.wordId) {
        const snapshot = await repository.getDashboardSnapshot(now);
        const current = snapshot.knowledgeStates.find((state) => state.itemId === activeCard.wordId);
        await repository.upsertKnowledgeState(recordMeaningResult(current, activeCard.wordId, graded.correct, now));
      }
  ...
  const submitCloze = async () => {
    ...
    try {
      ...
      await repository.upsertKnowledgeState(recordSpellingResult(current, activeWord.id, correct, now));
      await repository.upsertReviewCard(updated);
      await completeDailyTask(repository, 'review', currentStudyDate);
      setCards((items) => items.map((item) => item.id === updated.id ? updated : item));

...
describe('VocabularyTranslationCheck', () => {
  it('keeps covered words learning and sends missed words to spelling review', async () => {
    ...
    expect(learningRepository.upsertKnowledgeState).toHaveBeenCalledWith(expect.objectContaining({ itemId: 'v1', status: 'learning' }));
    expect(learningRepository.upsertKnowledgeState).toHaveBeenCalledWith(expect.objectContaining({ itemId: 'v2', status: 'review' }));
    expect(learningRepository.upsertReviewCard).toHaveBeenCalledWith(expect.objectContaining({ wordId: 'v2', format: 'word-cloze', lastCorrect: false }));
  });

...
export function SubjectiveEditor({ question, kind, repository = defaultRepository, onSubmit, revealFeedback = true }: { question: SubjectiveQuestion; kind: 'writing' | 'translation'; repository?: LearningRepository; onSubmit?: (body: string, feedback: SubjectiveFeedback) => void; revealFeedback?: boolean }) {
  ...
  return <section className="subjective-editor">
    ...
    {submitted && revealFeedback && <section><h2>针对性修改建议</h2><ul>{feedback.checks.map((item) => <li key={item.label}><strong>{item.passed ? '✓' : '待改'} {item.label}</strong>：{item.passed ? '已达到基础要求。' : item.suggestion}</li>)}</ul><p>{feedback.disclaimer}</p><h2>自查清单</h2><ul>{question.rubric.map((item) => <li key={item}>{item}</li>)}</ul><h2>参考答案</h2><p>{question.referenceAnswer}</p></section>}
  </section>;
}

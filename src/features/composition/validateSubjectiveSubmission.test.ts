...
describe('validateSubjectiveSubmission', () => {
  it('rejects empty and short writing without discarding the response', () => {
    expect(validateSubjectiveSubmission('writing', '')).toEqual({ valid: false, message: '请先完成写作，至少输入 80 个英文单词。' });
    expect(validateSubjectiveSubmission('writing', 'Practice is useful.')).toEqual({ valid: false, message: '当前 3 词，至少需要 80 个英文单词才能完成任务。' });
  });

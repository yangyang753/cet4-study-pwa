...
export function ExerciseRunner({ setId, kind, limit = 5, mode = 'practice', repository = defaultRepository, userId = 'local-learner' }: { setId?: string; kind?: PracticeKind; limit?: number; mode?: 'practice' | 'exam'; repository?: LearningRepository; userId?: string }) {
  ...
  if (finished) {
    ...
    return <section className="practice-summary"><h1>练习完成</h1><p>完成 {answered.length} 题，答对 {correct} 题，用时 {durationSeconds} 秒。</p><p>错题 {answered.filter((item) => item.correct === false).length} 道，已经加入复习安排。</p></section>;
  }
